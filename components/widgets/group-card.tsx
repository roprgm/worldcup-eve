...
import { FlagImage, LiveBadge, LiveDot } from "@/components/widgets/primitives";

// Presentation-only group card. The caller owns sorting, standings math,
// prediction logic, and live/final status mapping.

export type ResultStatus = "predicted" | "final" | "live";
export type Marker = "advance" | "third" | "none";
...
function ResultCell({ result }: { result: GroupCardResult }) {
  ...
  return (
    <div
      className={cn(
        "w-full select-none rounded py-1.5 text-center text-[11px] leading-none tabular-nums",
        live
    ...
    >
      <span className="relative inline-block rounded px-0.5">
        {live && (
          <span
            aria-hidden
            className="absolute -inset-x-1 -inset-y-0.5 animate-pulse rounded bg-rose-500/10"
          />
        )}
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-surface-border bg-card">
      ...
      <div
        ...
        style={{ gridTemplateColumns: groupGridColumns(columns.length) }}
      >
        <span />
        {columns.map((code) => (
          <span
            key={`head-${code}`}
            className="text-center text-[11px] font-medium uppercase leading-4 tracking-wide text-muted-foreground/75"
          >
            {code}
          </span>
        ))}
        <span className="text-center text-[11px] font-medium uppercase leading-4 tracking-wide text-muted-foreground/75">
          GD
        </span>
        <span className="text-center text-[11px] font-medium uppercase leading-4 tracking-wide text-muted-foreground/75">
          Pts
        ...
        {rows.flatMap((row) => [
          ...
          >
            <span className="w-3 text-right text-[10px] tabular-nums text-muted-foreground">
              {row.position}
            </span>
