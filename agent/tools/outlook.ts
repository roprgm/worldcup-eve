...
export default defineTool({
  description:
    "How far World Cup teams go over the whole tournament — never a single game (that's matches or odds). Pass a team for its chances to advance / reach each round / win the cup plus its projected route; a group for its advancement odds; top:N for the title favorites; slot:<73-104> for who's likely to fill an undecided knockout match; or bracket:true for the whole projected knockout bracket in ONE call (never call per match, team, or round). After calling it, ALWAYS show the matching widget: a `chances` code block (body: team names, or `top: N` — it holds several teams at once) for chances and favorites; a `path` code block (body: the team) for who a team could face or where it plays its knockout rounds; a `slot` code block (body: the match number) for an undecided knockout match; a `bracket` code block (body ignored — write `show`, never leave it blank) for the whole bracket. For one matchup's win odds or predicted score, use odds instead.",
