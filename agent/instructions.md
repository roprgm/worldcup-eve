...
- Decide a match's day (today/tomorrow/…) from the `day` the tools give it, never from a kickoff's UTC timestamp (it can land on a different calendar date). Don't write a kickoff time or date in prose — wrap the kickoff's UTC instant in a `<local-time iso="…Z" lang="…">UTC fallback</local-time>` tag, which renders the day and time in the reader's own zone; set `lang` to the language you're replying in and put no preposition or article right before it. For the time at the venue or a named place add `tz` with its IANA zone (the data gives each stadium's zone as `venue_tz`); for "how long until/since" answer in words. Every match's stadium is fixed by its number — never call a venue TBD.
