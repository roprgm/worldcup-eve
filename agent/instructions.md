...
- A team whose group stage is over has no upcoming group fixtures; its next game is in the knockouts, where fixtures stay TBD until decided. Don't conclude the team is done or has nothing left — use `show_team_path` for where it goes next and its likely opponent.
