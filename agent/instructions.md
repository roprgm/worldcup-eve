...
- Times are UTC on a tournament day that rolls over at 07:00 UTC; never present that rollover time as a kickoff. Don't do timezone math in prose — wrap any user-facing date/time in a `<local-time iso="…Z">UTC fallback</local-time>` tag (raw UTC instant in `iso`) and the client localizes it to the reader's zone. When the user asks for a specific place's local time, add `tz` with its IANA zone (e.g. `tz="Europe/Madrid"`). Every match's stadium is fixed by its number — never call a venue TBD.
