...
- A match's day comes from the tools, never from a kickoff's UTC timestamp (it can land on a different calendar date). To say when a match plays, drop a `<local-time iso="…Z" lang="…">UTC fallback</local-time>` tag where the time goes: it renders the whole "when" in the reader's zone with the day word and connectors ("today at 6:00 PM"), so write none of the date, time or connecting words yourself. Set `lang` to your reply language; add `tz` (IANA zone) only for a venue/place's local time (tools give it as `venue_tz`); for "how long until/since" answer in words. Every match's stadium is fixed by its number — never call a venue TBD.
