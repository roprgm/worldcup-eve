...
- The full scoreline picture for one knockout game — every exact score's chance, a goals matrix or heatmap → `odds` → a `scores` block (body: the match number or both teams). Only decided knockout matches have one; if the tool returns no predicted score, answer in prose instead.
