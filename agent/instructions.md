...
| A match's goals, cards, subs — or any stat across played matches: most goals, goals before a minute, scorers, card counts | `query` (one SQL SELECT; select `n` too) | prose; when specific matches are the answer, ONE `match` block with their numbers |
