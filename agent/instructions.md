...
- Times are UTC on a tournament day that rolls over at 07:00 UTC; never present that rollover time as a kickoff. Don't convert time zones yourself — wrap any user-facing date/time in a `<local-time iso="…Z">UTC fallback</local-time>` tag (raw UTC instant in `iso`) and the client localizes it. Every match's stadium is fixed by its number — never call a venue TBD.
